...
export default function StepConversion() {
  ...
  return (
    <FormSection title="5단계: 불법 전용 확인">
      <FormYesNo label="전용 여부" value={conversionYn} onChange={(v) => setField('conversionYn', v)} />
      ...
      {conversionYn === true && (
        <>
          ...
          <FormSelect label="전용 허가" items={PERMIT_STATUS} value={conversionPermitted} onChange={(v) => setField('conversionPermitted', v)} columns={3} />
        </>
      )}
    </FormSection>
  );
